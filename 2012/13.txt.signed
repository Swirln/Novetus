%MgmxzgmsPB3UcMLj3wYOkSOtbJsnF8I/Q2PguW7Zp3gKE5GIm1qUzyTGgBs16GXCrbZXVB9/EYfxZseJ7qCfooUsxcWpC5Jqvff2QD1KjoP3CgvH1QpYjoSKE5ZRem3JHWoeGXtMJiubYX2ApRkqv2EFmFkfNFkXv6Tp7EP1YQk=%%13%
...
function saveGearPositions()
	if not canSaveLoadout then return end
...
local addingPlayerChild = function(child, equipped, addToSlot, inventoryGearButton)
	...
	gearClone.MouseEnter:connect(function()
		pcall(function() if gearClone.GearReference and gearClone.GearReference.Value["ToolTip"] and gearClone.GearReference.Value.ToolTip ~= "" then
...
player.CharacterAdded:connect(function()
	--canSaveLoadout = false -- we need to save loadout table until we check it out
	...
	for i = 1, #backpackChildren do
		...
		if loadoutCount >= 10 then -- we've accounted for all slots, no need to go thru rest of gear
			break
		end
	end]]
	--canSaveLoadout = true -- used loadout table, allow script to edit this again
...
game:GetService("GuiService").KeyPressed:connect(function(key) activateGear(key) end)
