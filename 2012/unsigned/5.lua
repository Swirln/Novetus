%MgmxzgmsPB3UcMLj3wYOkSOtbJsnF8I/Q2PguW7Zp3gKE5GIm1qUzyTGgBs16GXCrbZXVB9/EYfxZseJ7qCfooUsxcWpC5Jqvff2QD1KjoP3CgvH1QpYjoSKE5ZRem3JHWoeGXtMJiubYX2ApRkqv2EFmFkfNFkXv6Tp7EP1YQk=%%5%
...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	if hasGraphicsSlider then
		...
		local function setGraphicsQualityLevel(newLevel)
			...
			pcall(function() UserSettings().GameSettings.SavedQualityLevel = newLevel end)
		...
		local function goToAutoGraphics()
			...
			if not UserSettings().GameSettings:InStudioMode() then -- only save automatic setting if we are in game and we go auto
				pcall(function() UserSettings().GameSettings.SavedQualityLevel =  Enum.SavedQualitySetting.Automatic end)
			end
			settings().Rendering.QualityLevel = Enum.QualityLevel.Automatic
		end
		
		local function translateSavedQualityLevelToInt(savedQualityLevel)
			local success = pcall(function() local test = Enum.SavedQualitySetting.Automatic end)
		...
		local function goToManualGraphics(explicitLevel)
			...
			settings().Rendering.QualityLevel = math.min(settings().Rendering:GetMaxQualityLevel() - 1, math.max(1,graphicsLevel.Value * (settings().Rendering:GetMaxQualityLevel() - 1)))
		...
		if UserSettings().GameSettings:InStudioMode() then
			...
			local success = pcall(function() local test = UserSettings().GameSettings.SavedQualityLevel end)
			if success then
